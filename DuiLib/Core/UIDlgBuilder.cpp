...
// 			CTreeNodeUI* pNode			= new CTreeNodeUI();
			CTreeNodeUI* pParentNode	= static_cast<CTreeNodeUI*>(pParent->GetInterface(_T("TreeNode")));
			CTreeViewUI* pParentTree	= static_cast<CTreeViewUI*>(pParent->GetInterface(_T("TreeView")));
			CTreeViewUI* pParent = NULL;
			if(pParentNode)
				pParent = pParentNode->GetTreeParent();
			else if(pParentTree)
				pParent = pParentTree;
			CTreeNodeUI* pNode			= new CTreeNodeUI(pParent, pParentNode);
			...
			case 11:
				...
				break;
 			case 12://HorSlidePage VerSlidePage
 				if (_tcscmp(pstrClass, DUI_CTR_HORSLIDEPAGE) == 0)			  pControl = new CHorSlidePageUI;
 				if (_tcscmp(pstrClass, DUI_CTR_VERSLIDEPAGE) == 0)			  pControl = new CVerSlidePageUI;
 				break;
 			case 13://SlidePageItem
 				if (_tcscmp(pstrClass, DUI_CTR_SLIDEPAGEITEM) == 0)			  pControl = new CSlidePageItemUI;
 				break;
            case 14:
