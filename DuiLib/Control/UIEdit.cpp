...
namespace DuiLib
{
	...
	void CEditUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_KILLFOCUS && IsEnabled() && !IsReadOnly() ) 
		{
			Invalidate();
		}
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK || event.Type == UIEVENT_RBUTTONDOWN) 
		{
			if( IsEnabled() ) {
				...
				if( IsFocused() && m_pWindow == NULL && !IsReadOnly() )
				{
					m_pWindow = new CEditWnd();
					ASSERT(m_pWindow);
					m_pWindow->Init(this);
				...
				else if( m_pWindow != NULL && !IsReadOnly() )
				{
#if 1
					int nSize = GetWindowTextLength(*m_pWindow);
					if( nSize == 0 )
				...
				else if(IsReadOnly())
				{
					if(event.Type == UIEVENT_BUTTONDOWN)
						m_pManager->SendNotify(event.pSender, DUI_MSGTYPE_BUTTONDOWN);
					else if(event.Type == UIEVENT_RBUTTONDOWN)
						m_pManager->SendNotify(event.pSender, DUI_MSGTYPE_RBUTTONDOWN);
					else if(event.Type == UIEVENT_DBLCLICK)
						m_pManager->SendNotify(event.pSender, DUI_MSGTYPE_DBCLICK);
				}
			}
			return;
		}
